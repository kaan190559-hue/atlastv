...
const ADMIN_PASSWORD = '190559'
const VOD_M3U_URL = 'https://file.garden/Z-hq5n4Shk27aY58/Wars-vod-iptv.m3u'
const LIVE_M3U_FILE = process.env.ATLAS_LIVE_M3U_FILE ?? 'C:\\Users\\user\\Downloads\\vavoo_full_worker.m3u'
const ADMIN_SETTINGS_FILE = resolve(process.cwd(), '.atlas', 'admin-settings.json')
...
const defaultAdminSettings: ServerAdminSettings = {
  vodM3uUrl: VOD_M3U_URL,
  liveM3uUrl: '',
  sportsM3uUrl: '',
}

const catalogCache = new Map<string, ServerContentItem[]>()
const catalogPromise = new Map<string, Promise<ServerContentItem[]>>()
const liveCatalogCache = new Map<string, ServerContentItem[]>()
const liveCatalogPromise = new Map<string, Promise<ServerContentItem[]>>()

function normalizeAdminSettings(settings: Partial<ServerAdminSettings> = {}): ServerAdminSettings {
  return {
...
async function readAdminSettings(): Promise<ServerAdminSettings> {
  try {
    const raw = await readFile(ADMIN_SETTINGS_FILE, 'utf8')
    return { ...defaultAdminSettings, ...(JSON.parse(raw) as Partial<ServerAdminSettings>) }
  } catch {
    return defaultAdminSettings
  }
}

async function writeAdminSettings(settings: ServerAdminSettings) {
...
function readJsonBody(req: IncomingMessage) {
  return new Promise<{ password?: string; reset?: boolean; settings?: Partial<ServerAdminSettings> }>((resolve, reject) => {
    let body = ''
...
function atlasMediaProxy(): Plugin {
  return {
    name: 'atlas-media-proxy',
    configureServer(server) {
      server.middlewares.use(ADMIN_SETTINGS_PATH, async (req, res) => {
        try {
          if (req.method === 'GET') {
          ...
          console.error('Atlas admin settings error:', error)
          res.statusCode = 500
          res.end('Admin settings failed')
        }
      })

      server.middlewares.use(CATALOG_PATH, async (req, res) => {
        try {
          const requestUrl = new URL(req.url ?? '', 'http://atlas.local')
          ...
          res.end(JSON.stringify({ items: page, total: grouped.length }))
        } catch (error) {
          console.error('Atlas catalog error:', error)
          res.statusCode = 502
          res.end('Atlas catalog failed')
        }
      })

      server.middlewares.use(LIVE_CATALOG_PATH, async (req, res) => {
        try {
          const requestUrl = new URL(req.url ?? '', 'http://atlas.local')
          ...
          const filtered = catalog.filter((item) => {
            ...
            if (
              query &&
              !`${item.title} ${itemCountry} ${itemCategory}`.toLocaleLowerCase('tr-TR').includes(query)
            ) {
              return false
            }
            return true
          ...
          res.end(JSON.stringify({ items: page, total: filtered.length, countries, categories }))
        } catch (error) {
          console.error('Atlas live catalog error:', error)
          res.statusCode = 502
          res.end('Atlas live catalog failed')
        }
      })
      ...
      server.middlewares.use(PROXY_PATH, async (req, res) => {
        try {
          ...
          console.error('Atlas media proxy error:', error)
          res.statusCode = 502
          res.end('Atlas media proxy failed')
        }
      })
    },
  }
...
function loadLiveServerCatalog(sourceUrl = '', refreshKey = '') {
  const sourceKey = sourceUrl || LIVE_M3U_FILE
  ...
  const playlistPromise = sourceUrl
  ...
  const pendingLoad = playlistPromise
    .then((playlist) => {
      const parsed = parseLivePlaylist(playlist, sourceKey)
...
function parseLivePlaylist(playlist: string, sourceKey = 'live'): ServerContentItem[] {
  const lines = playlist
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean)
  const items: ServerContentItem[] = []
...
function parseVodPlaylist(playlist: string, sourceKey = 'vod'): ServerContentItem[] {
  const lines = playlist
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean)
...
function groupCatalogItems(items: ServerContentItem[]) {
  const groups = new Map<string, ServerContentItem[]>()
...
function getTitleFromExtinf(line: string) {
...
function getEpisodeInfo(title: string) {
  const seasonEpisode =
...
function getLiveDisplayTitle(title: string) {
  return title
    .replace(/\s+\.[a-z]$/i, '')
    .replace(/\s+/g, ' ')
    .trim()
...
function inferLiveCategory(title: string) {
  ...
  if (/(kids|çocuk|cocuk|cartoon|disney|nick|boomerang|minika|baby|junior|toons)/i.test(source)) {
    return 'Çocuk'
  }
  if (/(music|muzik|müzik|mtv|radio|vh1|club|hit|hits)/i.test(source)) {
    return 'Müzik'
  }
  if (/(discovery|animal|history|nat geo|national geographic|docu|doku|science|planet|wild)/i.test(source)) {
    return 'Belgesel'
  }
  if (/(islam|quran|kuran|mekke|medine|religion|diyanet)/i.test(source)) {
    return 'Dini'
  }
  return 'Genel'
}

function getLivePlaceholder(category: string, variant: 'poster' | 'backdrop') {
...
function getLiveImage(country: string, title: string, category: string, variant: 'poster' | 'backdrop') {
  if (country === 'Turkey') {
    return getTurkishChannelLogo(title) ?? getChannelInitialImage(title, variant)
  }

  return getLivePlaceholder(category, variant)
}

function getTurkishChannelLogo(title: string) {
  const normalized = title
    ...
    .replace(/\s+/g, ' ')
    .trim()
  const logoByName: Array<[RegExp, string]> = [
...
function getChannelInitialImage(title: string, variant: 'poster' | 'backdrop') {
...
function inferType(groupTitle: string, title: string): 'movie' | 'series' {
...
function rewritePlaylist(playlist: string, playlistUrl: string, userAgent: string, referer = '', origin = '') {
  ...
  return playlist
    ...
    .map((line) => {
      ...
      const tagLine = line.replace(/URI="([^"]+)"/g, (_match, uri: string) => `URI="${toProxyUrl(uri)}"`)
